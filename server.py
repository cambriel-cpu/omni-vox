...
async def handle_message(message: dict, session: VoiceSession):
    ...
    if message_type == "ping":
        ...
        # Keepalive response from client - no action needed
        pass
        ...
        sonos_speaker = message.get("sonos_speaker", None)
        sonos_location = message.get("sonos_location", "local")
        sonos_volume = message.get("sonos_volume", None)
...
async def call_openclaw(message: str, timeout: float = 45.0, model: str = None) -> str:
    ...
    # Use main session for persistent logging instead of hook sessions
    # Hook sessions don't create files in /sessions that polling can find
    session_key = None  # Use main session
    ...
    # Inject conversation history if available (use consistent key for conversation tracking)
    conv_session_key = "voice"  # Simple key for conversation history tracking
    ...
    # Note: No sessionKey specified = uses main session which creates persistent files
    if model:
...
async def generate_tts(text: str, provider: str = "kokoro") -> bytes:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        if provider == "elevenlabs":
            ...
            response = await client.post(
                KOKORO_URL,
                json={"model": KOKORO_MODEL, "input": text, "voice": KOKORO_VOICE, "response_format": "opus"},
            )
        response.raise_for_status()
...
async def voice_interaction(
    ...
    try:
        ...
        # Use consistent session key for conversation tracking
        conv_session_key = "voice"
...
async def clear_conversation(llm_model: Optional[str] = Form(None)):
    ...
    # Use consistent session key for conversation tracking
    conv_session_key = "voice"
    conversation.clear(conv_session_key)
    return {"cleared": conv_session_key, "status": "ok"}
...
async def get_conversation_history(llm_model: Optional[str] = None):
    ...
    # Use consistent session key for conversation tracking
    conv_session_key = "voice"
    turns = conversation.get_history(conv_session_key)
    return {
        "session_key": conv_session_key,
...
if __name__ == "__main__":
    ...
    if os.path.exists(ssl_keyfile) and os.path.exists(ssl_certfile):
        print("🔒 Starting OmniVox with HTTPS support")
        print("📍 HTTPS: https://0.0.0.0:7100")
        print("📍 HTTP:  http://0.0.0.0:7101 (fallback)")
        ...
        def run_https():
            uvicorn.run(app, host="0.0.0.0", port=7100, ssl_keyfile=ssl_keyfile, ssl_certfile=ssl_certfile)
        
        # HTTP server (fallback) 
        def run_http():
            uvicorn.run(app, host="0.0.0.0", port=7101)
        
        # Start HTTPS in main thread
        ...
        print("📍 Starting OmniVox with HTTP only (no SSL certificates found)")
        print("📍 HTTP: http://0.0.0.0:7100")
