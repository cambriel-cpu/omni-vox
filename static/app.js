// Omni Vox — Push-to-talk voice client v2.5.7
...
async function initializeWebSocketMode() {
    ...
    debugLog('🚀 Initializing WebSocket mode');
    try {
        ...
        wsClient.onTranscript = (text) => {
            debugLog(`✅ Transcription received: "${text}"`, 'success');
            updateStatus(`You said: "${text}"`);
        };
        
        wsClient.onResponseText = (text) => {
            debugLog(`✅ LLM response received (${text.length} chars)`, 'success');
            updateStatus(`Omni: ${text.substring(0, 100)}${text.length > 100 ? '...' : ''}`);
        };
        
        wsClient.onAudioStart = () => {
            debugLog(`🔊 Audio streaming started, player ready: ${!!wsClient.audioPlayer}`, 'success');
            updateInterruptButton(true);
            setStatus('speaking', 'Streaming audio...');
        };
        
        wsClient.onAudioEnd = () => {
            debugLog('✅ Audio streaming complete', 'success');
            updateInterruptButton(false);
            setStatus('', 'Ready');
            updateStatus('Ready for next message');
        };
        
        wsClient.onSonosPlayed = (message) => {
            debugLog(`🔊 Audio played on ${message.speaker} (${message.location})`, 'success');
            updateInterruptButton(false);
            setStatus('', 'Ready');
            updateStatus('Ready for next message');
        };
        ...
        // Skip audio initialization during page load - will happen on first user gesture
        debugLog('⏳ Audio initialization deferred until first user interaction');
...
async function initializeAudioComponents() {
    try {
        debugLog('🎵 Creating OpusStreamingPlayer...');
        
        // Always create a new audio player (don't reuse failed instances)
        streamingAudioPlayer = new OpusStreamingPlayer();
        const audioReady = await streamingAudioPlayer.initialize();
        
        if (audioReady) {
            debugLog('✅ OpusStreamingPlayer initialized successfully', 'success');
            
            // Link audio player to WebSocket client
            if (wsClient) {
                wsClient.audioPlayer = streamingAudioPlayer;
                debugLog('✅ Audio player linked to WebSocket client', 'success');
            } else {
                debugLog('❌ WebSocket client not found for audio player linking', 'error');
            }
            updateStatus('Audio streaming ready');
            return true;
        } else {
            debugLog('❌ OpusStreamingPlayer initialization failed', 'error');
            streamingAudioPlayer = null; // Clear failed instance
            return false;
        }
    } catch (error) {
        debugLog(`❌ Audio initialization exception: ${error.message}`, 'error');
...
async function ensureAudioReady() {
    // Check if we need to initialize audio components
    if (!streamingAudioPlayer) {
        debugLog('🎵 Audio player missing - initializing now...');
        const audioReady = await initializeAudioComponents();
        if (!audioReady) {
            debugLog('❌ Audio initialization failed - WebSocket streaming will not work', 'error');
            return false;
        }
    } else if (!streamingAudioPlayer.audioContext || streamingAudioPlayer.audioContext.state !== 'running') {
        debugLog('🎵 AudioContext not running - attempting to resume...');
        try {
            await streamingAudioPlayer.audioContext.resume();
            if (streamingAudioPlayer.audioContext.state === 'running') {
                debugLog('✅ AudioContext resumed successfully', 'success');
            } else {
                debugLog('❌ AudioContext still not running', 'error');
                return false;
            }
        } catch (error) {
            debugLog(`❌ Failed to resume AudioContext: ${error.message}`, 'error');
            return false;
        }
    } else {
    ...
    // Final check
    if (wsClient && !wsClient.audioPlayer) {
        wsClient.audioPlayer = streamingAudioPlayer;
        debugLog('🔗 Re-linked audio player to WebSocket');
    }
...
function startRecording() {
    ...
    debugLog('🎙️ Started recording audio');
    audioChunks = [];
...
function stopRecording() {
    return new Promise((resolve) => {
        ...
        mediaRecorder.onstop = () => {
            ...
            debugLog(`🔇 Stopped recording, audio blob size: ${blob.size} bytes`);
            audioChunks = [];
            isRecording = false;
...
async function processRecordingWebSocket(audioBlob) {
    if (!wsClient || !wsClient.isConnected) {
        debugLog('❌ WebSocket not connected', 'error');
    ...
    try {
        ...
        debugLog(`📡 Sending voice request via WebSocket (${base64Audio.length} chars)`);
        // Send via WebSocket with selected TTS provider, LLM model, and speaker
        const selectedTTSProvider = ttsSelect.value || 'kokoro';
        const selectedLLMModel = llmSelect.value || null;
        const selectedSpeaker = speakerSelect.value || 'none';
        const selectedVolume = parseInt(volumeInput.value) || 65;
        
        debugLog(`🎤 Using TTS: ${selectedTTSProvider}, LLM: ${selectedLLMModel || 'default'}, Speaker: ${selectedSpeaker}, Volume: ${selectedVolume}`);
        await wsClient.sendVoiceRequest(base64Audio, selectedTTSProvider, selectedLLMModel, selectedSpeaker, selectedVolume);
        
        debugLog('⏳ Waiting for WebSocket response...');
        updateStatus("Processing voice (streaming)...");
        
    } catch (error) {
        debugLog(`❌ WebSocket processing error: ${error.message}`, 'error');
...
async function handleTalkStart(e) {
    ...
    // ALWAYS initialize audio components on first user gesture
    if (wsClient && wsClient.isConnected) {
        if (!streamingAudioPlayer) {
            debugLog('🎵 First user gesture - initializing audio components...', 'info');
            try {
                const audioReady = await initializeAudioComponents();
                if (audioReady && streamingAudioPlayer && wsClient) {
                    debugLog('✅ Audio player successfully linked to WebSocket', 'success');
                    wsClient.audioPlayer = streamingAudioPlayer; // Ensure linkage
                } else {
                    debugLog('⚠️ Audio player initialization incomplete but continuing...', 'warning');
                ...
                // Don't prevent recording attempt - maybe it will work anyway
            }
        } else {
            debugLog('✅ Audio components already initialized', 'success');
            // Ensure audio context is ready after user gesture
            if (streamingAudioPlayer.audioContext?.state === 'suspended') {
                debugLog('🎵 Resuming suspended AudioContext after user gesture...', 'info');
                try {
                    await streamingAudioPlayer.audioContext.resume();
                    debugLog('✅ AudioContext resumed successfully', 'success');
                } catch (resumeError) {
                    debugLog('⚠️ AudioContext resume failed, but continuing...', 'warning');
                }
            }
        }
    }
...
async function initializeApp() {
    ...
    // Discover speakers, TTS providers, and LLM models after DOM is ready
    debugLog('🔍 Discovering speakers, TTS providers, and LLM models...');
    await Promise.all([
        discoverSpeakers(),
        discoverTTSProviders(), 
        discoverLLMModels()
    ]);
    debugLog('✅ All discoveries complete');
}
